...
class Base(object):
    ...
    def __init__(self, net, cfg, data_iter=None, log_name='logs.txt'):
        ...
        tfconfig.graph_options.rewrite_options.remapping = RewriterConfig.OFF  # 必须显式关闭remap
        ...
        train_log_dir="./train_logs_tb"
        self.tf_summary_writer = tf.summary.FileWriter(train_log_dir)
             ...
             with slim.arg_scope([slim.conv2d, slim.conv2d_in_plane, \
                ...
                with tf.control_dependencies(extra_update_ops):
                        ...
                        for grad,var in grads:
                            if grad is not None:
                                tf.summary.scalar(var.name,tf.global_norm([grad])) 
                        train_op = self._optimizer.apply_gradients(grads)
        
        for var in tf.trainable_variables():    
            tf.summary.scalar(var.name + "_weight",tf.global_norm([var])) 
        return train_op
...
#         return train_op
    ...
    def train(self):
        ...
        for itr in range(start_itr, end_itr):
            ...
            # train one step
            self.gpu_timer.tic()
            _, self.lr_eval, *summary_res, tf_summary = self.sess.run(
                [self.graph_ops[0], self.lr, *self.summary_dict.values(),merge_summary], feed_dict=feed_dict)
            self.gpu_timer.toc()
            if itr % 10 ==0:
                self.tf_summary_writer.add_summary(summary=tf_summary, global_step=itr)
            itr_summary = dict()
...
class Tester(Base):
    ...
    def _make_graph(self):
        ...
        #     #f.write("imgs shape {}, input_pose_coords shape {}, input_pose_valids shape {}".format(imgs.shape,input_pose_coords.shape,input_pose_valids.shape))
           
        #     f.write("{}".format(self._input_list))
        #     f.write("{}".format(self._output_list))
    ...
    def predict_one(self, data=None):
        ...
        #         f.write("{} shape {} \n".format(k, v.shape))
        self.gpu_timer.tic()
        #res = self.sess.run(list(feed_dict.keys()),feed_dict=feed_dict)
