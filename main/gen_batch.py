            ...
            for j in range(cfg.num_kps):
                if total_joints[i,j,2] > 0:
                    ...
                    total_joints[i,j,2] *= ((total_joints[i,j,0] >= 0) & (total_joints[i,j,0] < cfg.input_shape[1]) & (total_joints[i,j,1] >= 0) & (total_joints[i,j,1] < cfg.input_shape[0])
                     & ( total_joints[i,j,0].astype(np.float) / cfg.input_shape[1] * cfg.output_shape[1] < cfg.output_shape[1] - 2)
                     &  ( total_joints[i,j,1].astype(np.float) / cfg.input_shape[0] * cfg.output_shape[0] < cfg.output_shape[0] - 2))
