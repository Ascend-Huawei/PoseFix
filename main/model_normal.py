...
from nets.basemodel import resnet50, resnet101, resnet152, resnet_arg_scope, resnet_v1,resnet_arg_scope_nobn
...
class Model(ModelDesc):
        ...
        with slim.arg_scope(resnet_arg_scope(bn_is_training=is_training)):
            ...
            with tf.variable_scope('out'):
                out = slim.conv2d(out, cfg.num_kps, [1, 1],
                    ...
                    padding='SAME', normalizer_fn=None, activation_fn=None,biases_initializer=None
                    )
                bias = tf.get_variable(name="bias", shape=[cfg.num_kps], initializer=tf.constant_initializer(0.))
    ...
    def make_network(self, is_train):
        ...
        if is_train:
            ...
            tf.summary.scalar("heatloss", loss_heatmap)
            tf.summary.scalar("coordloss", loss_coord)
